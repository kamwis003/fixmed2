...
import type { IInvoice, IInvoicesResponse, IInvoiceQueryParams, IInvoicePagination } from '@/types/billing'
 ...
 */
export const createCustomerPortalSession = async (): Promise<ICreateCustomerPortalSessionResponse> =>
...
export const useBilling = (_customerId: string): IUseBillingReturn => {
  ...
  const fetchInvoices = useCallback(async (params: IInvoiceQueryParams = {}) => {
    if (!user) return
